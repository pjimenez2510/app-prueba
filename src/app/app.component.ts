...
import { Assignment, Semester } from './models/models';
...
export class AppComponent implements OnInit {
  ...
  startReminderInterval() {
    this.reminderInterval = setInterval(() => {
      this.testData.forEach((data) => {
        data.subjects.forEach((assignment) => {
          assignment.assignments.forEach((assignment) => {
            this.showReminderToast(assignment);
          });
        });
      });
    }, 60000);
  ...
  async showReminderToast(assignment: Assignment) {
    const toast = await this.toastController.create({
      message: `¡Ey tienes una tarea pendiente: ${assignment.title}`,
      duration: 3000,
      color: 'warning',
      buttons: [
        {
